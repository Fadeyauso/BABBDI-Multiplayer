...
public class AchievementPopup : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (timer > 0)
        {
            if (transform.position.y < upPos.transform.position.y) transform.position += new Vector3(0,1,0);
        }

        if (timer < 0)
        {
            if (transform.position.y > initPos.transform.position.y) transform.position -= new Vector3(0,1,0);
        }
    }
