...
namespace BabbdiMultiplayer
{
    public static class SendData
    {
        ...
        public static void SendDropItem(int itemId,Vector3 pos, Vector3 forward)
        {
            using (Packet _packet = new Packet((int)Packets.DropItem))
            {
        ...
        public static void SendPickup(Vector3 pos,int item)
        {
            using (Packet _packet = new Packet((int)Packets.Pickup))
            {
                _packet.Write(pos);
                _packet.Write(item);
                Networking.SendTCPData(_packet);
